...
void copy_frame_to_vmaf_file(int cur_frame,long frame_size,FILE *fp_org,FILE *fp_vmaf)
{
    int i;
    char *tmpbuf;

    tmpbuf = (char *)malloc(frame_size);
 
    fseek(fp_org,0,SEEK_SET);
    for(i=0;i<cur_frame;i++){
        fseek(fp_org,frame_size,SEEK_CUR);
    }

    fread(tmpbuf,1,frame_size,fp_org);
    fwrite(tmpbuf,1,frame_size,fp_vmaf);
...
main(int argc, char *argv[])
{
   ...
   /* Loop once for each frame named on command line */
   {

      /* Open image from file or stream (might contain multiple pages) */
      filePath = opt.degFile;

      FILE *fp_degrad = fopen(filePath,"rb");
      width = opt.wd;
      height = opt.ht;
      unsigned long frame_size = 3*width*height;
      unsigned long read_size = frame_size;
      ...
      fp_org = fopen(opt.orgFile,"rb");
      fp_vmaf = fopen(opt.vmafRefFile,"wb");

      if(fp_org == NULL || fp_vmaf == NULL || fp_degrad == NULL){
        printf(" not able to open file ... Exiting !!! ");
        exit(1);
      }

      // loop untill we complete all the frames
      while(frame_size == read_size) {

         read_size = fread(pxl,1,frame_size,fp_degrad);
         if(read_size != frame_size)
         {
             printf(" file ended. break !!! ");
             break;
         }
         ...
         for(;;) {
            ...
            if(msg != NULL) {
               ...
               cur_frame = get_frame_num_from_msg(msg);
               imgScanCount++;
...
//               break;
         ...
         if(cur_frame > 0){
             printf("cur_fram = %d ... dumping to rgb \n", cur_frame);
             copy_frame_to_vmaf_file(cur_frame-1,frame_size,fp_org,fp_vmaf);
         } else {
             printf(" could not find frame num from \n");
         }
...
HandleArgs(UserOptions *opt, int *fileIndex, int *argcp, char **argvp[])
{
   ...
   struct option longOptions[] = {
         ...
         {"org-file",       required_argument,     NULL, 'o'},
         {"degraded-file",       required_argument,NULL, 'd'},
         {"vmafref-file",       required_argument,NULL, 'v'},
   ...
   *fileIndex = 0;
   ...
   for(;;) {
      optchr = getopt_long(*argcp, *argvp,
            "cw:h:o:d:V:e:E:g:lm:np:q:r:s:t:x:X:y:Y:vC:DMN:PRS:G:U", longOptions, &longIndex);
      if(optchr == -1)
         break;
      ...
      switch(optchr) {
         ...
         case 'c':
            ...
            break;
         case 'w':
            err = StringToInt(&(opt->wd), optarg, &ptr);
            if(err != DmtxPass || opt->wd <= 0 || *ptr != '\0')
               FatalError(EX_USAGE, _("Invalid image width specified \"%s\""), optarg);
            break;
         case 'h':
            err = StringToInt(&(opt->ht), optarg, &ptr);
            if(err != DmtxPass || opt->ht <= 0 || *ptr != '\0')
               FatalError(EX_USAGE, _("Invalid image height specified \"%s\""), optarg);
            break;
         case 'o':
            opt->orgFile = optarg;
            break;
...
OPTIONS:\n"), programName, programName);
  ...
  -w, --width                 width of RGB file\n\
  -h, --heigth                print height of RGB file\n\
...
int  get_frame_num_from_msg(DmtxMessage *msg)
{
   char numstr[20];
   int num,i;
   
   memcpy(numstr,msg->output, sizeof(char)*msg->outputIdx);
   numstr[msg->outputIdx] = '\0';

   for(i=0;i<msg->outputIdx;i++){
       if(msg->output[i] > '9' || msg->output[i] < '0')
       {
           return -1;
       }
   }
